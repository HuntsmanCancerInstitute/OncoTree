...
public class OncoTreeClassifier {
	...
	private boolean classifyNodes = true;
	...
	public OncoTreeClassifier (String[] args) {
		...
		try {
			...
			if (classifyNodes) tissueNodePromptBuilder = new TissueNodePromptBuilder(tissueCodeNodeCodes, tissueNodeCatalogDir, tissueNodeExampleDir);
			...
			if (classifyNodes) classifyTumorNodes();
	...
	private void classifyTumorTissues() throws Exception {
		...
		for (ClassifiedTumor ct: testIdClasTum.values()) {
			...
			//This will also set the oncoTreeTissueCode if "oncotree_tissue_code" is present in jo.
			//It does not test if it is a valid tissue code.
	...
	private void classifyTumorNodes() throws Exception {
		...
		HashMap<String, String> tissueCodes = tissueNodePromptBuilder.getTissueCodeNodeCodes();
		...
		for (ClassifiedTumor ct: testIdClasTum.values()) {
			...
			if (tissueCodes.containsKey(tissueCode)==false) {
				log.warn("\tTissue Code "+tissueCode+" not found, skipping "+ct.getTestOrderId());
				continue;
			}
		...
		log.info("""
				...
				**************************************************************************************
				...
				Options:
				  ...
				  -s Timeout in seconds for each query, defaults to 600
